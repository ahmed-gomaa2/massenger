...
const Messages = (props, ref) => {
    ...
    return (
        <div ref={messagesRef} className={'Messages'} style={props.formHeight && {height: `calc(100% - ${props.formHeight}px`}}>
           ...
           <Modal modalClosed={modelCloseHandler} show={showModal} >{currentModalImg && <img style={{width: '100%', height: '100%'}} src={'http://localhost:5000/files/' + currentModalImg?.iv + '/' + currentModalImg?.src} />}</Modal>
        </div>
    )
}
