            ...
            if(findRoomError) {
                ...
                connection.query('INSERT INTO messages (body, sender_id, receiver_id, room_id, create_at, hasFiles) VALUES (?, ?, ?, ?, ?, ?)', [bodyEncrypted, message.sender_id, message.receiver_id, room_id, new Date(), message.hasFiles], (createMessageError, createMessageRes) => {
                    if(createMessageError) {
                        ...
                        connection.query('UPDATE rooms SET updated_at = ? WHERE id = ?', [new Date(), message.room_id], (updateRoomError, updatedRoomRes) => {
                            if(updateRoomError) {
                                ...
                                if(message.hasFiles) {
                                    req.files.map((f, i) => {
                                        ...
                                        saveEncryptedFile(f, encryptFile, getEncryptedFilePath, f.buffer, path.join('./files', f.originalname));
                                    });
                                    const basePath = 'http://localhost:5000/files/';
                                    const values = [...req.files.map(f => [`${encrypt(f.filename)}`, f.mimetype, f.size, createMessageRes.insertId, f.iv])];
                                    console.log(values);
                                    // console.log([...req.files.map(f => [basePath + `${f.filename}`, f.mimetype, f.size, createMessageRes.insertId])])
                                    connection.query('INSERT INTO files (src, fileType, size, message_id, iv) VALUES ?', [values], (insertFileError, insertFileRes) => {
                                        if(insertFileError) {
                                            res.status(500).json({error: {type: 'server', msg: 'SOMETHING WENT WRONG WITH THE SQL SERVER!'}})
                                        } else {
                                            decrypt(values[0][0]);
                                            const files = [...values.map(v => ({src: decrypt(v[0]), fileType: v[1], size: v[2], message_id: v[3], iv: v[4]}))];
                                            console.log(files);
                                            const messageData = {
